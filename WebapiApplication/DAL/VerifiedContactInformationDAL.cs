...
namespace WebapiApplication.DAL
{
    public class VerifiedContactInformationDAL
    {
        public VerifiedContactInformationML DgetMobileEmailVerification(long? CustID)
        {
            ...
            {
                Commonclass.ApplicationErrorLog("[dbo].[Usp_GetContactVerificationForCustomer]", Convert.ToString(ex.Message), Convert.ToInt32(CustID), "DgetMobileEmailVerification", null);
            }
            finally
            {
                con.Close();
